...
def _corridor_density(
    ...
    for route in route_groups:
        ...
        if route_area_km2 > 0 and not route_simultaneous.empty:
            hotspot_density = max(hotspot_density, float(route_simultaneous.mean()) / route_area_km2)
...
def _complexity_components(
    ...
    return {
        ...
        "planned_route_crossings": int(_route_crossing_count(route_groups)),
...
def _route_crossing_count(route_groups: list[dict[str, Any]]) -> int:
    segments = []
    for route in route_groups:
        xy = _project(route["coordinates"])
        for start, end in zip(xy[:-1], xy[1:]):
            segments.append((start, end))

    crossings = 0
    for left_index, (a1, a2) in enumerate(segments):
        for b1, b2 in segments[left_index + 1 :]:
